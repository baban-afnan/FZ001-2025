...
class BvnUserController extends Controller
{
    public function index(Request $request)
    {
        ...
        // custom ordering: pending first, then query, then others
...
{
    $submission = BvnUser::findOrFail($id);
    ...
    if ($submission->status === 'query') {
        $validated = $request->validate(
            [
                'email' => 'required|email|max:500|unique:bvn_user,email,' . $id,
                'phone_no' => 'required|string|max:20|unique:bvn_user,phone_no,' . $id,
                'bvn' => 'required|string|size:11|regex:/^[0-9]{11}$/',
                'account_no' => 'required|string|max:20',
            ],
            [
                'email.unique' => 'This email is already in the user platform.',
                'phone_no.unique' => 'This phone number is already in the user platform.',
            ]
        );
