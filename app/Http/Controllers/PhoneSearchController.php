  ...
  public function index(Request $request)
{
    ...
    // Base query: load submissions with relations
    ...
    // ✅ Custom ordering: pending first, then processing, then others
    $query->orderByRaw("
        CASE 
    ...
    // Fetch active BVN search service
    $bvnService = Service::where('name', 'BVN SEARCH')
                    ->where('is_active', true)
                    ->first();

    // Load active modification fields for this service
    ...
    return view('bvn.phone-search', [
        ...
        'bvnService'         => $bvnService, // in case you need it in blade
    ]);
}
